...
import com.genymobile.scrcpy.video.ScreenFrameCapture;
...
public final class Server {
    ...
    private static void scrcpy_ws(Options options) throws IOException, ConfigurationException {
        if (Build.VERSION.SDK_INT < AndroidVersions.API_31_ANDROID_12 && options.getVideoSource() == VideoSource.CAMERA) {
            Ln.e("Camera mirroring is not supported before Android 12");
            throw new ConfigurationException("Camera mirroring is not supported");
        }
        ...
        Workarounds.apply();
            List<AsyncProcessor> asyncProcessors = new ArrayList<>();
        try {
            ControllerFrame controller = new ControllerFrame(cleanUp, options);
            ScreenFrameCapture surfaceCapture = new ScreenFrameCapture(controller, options);
            RecordingServer recordingServer = new RecordingServer(options.getPort());
            recordingServer.setController(controller);
            controller.setSurfaceCapture(surfaceCapture);
            controller.setRecordingServer(recordingServer);

            asyncProcessors.add(controller);

            SurfaceFrameEncoder surfaceFrameEncoder = new SurfaceFrameEncoder(surfaceCapture);

            StreamerVideo streamer = new StreamerVideo(
                    recordingServer,
                    options.getVideoCodec(),
                    options.getSendCodecMeta(),
                    options.getSendFrameMeta()
            );
            recordingServer.setStreamerVideo(streamer);
            SurfaceVideoEncoder surfaceVideoEncoder = new SurfaceVideoEncoder(surfaceCapture,streamer,options);
            surfaceFrameEncoder.setRecordingServer(recordingServer);
            controller.setSurfaceEncoder(surfaceFrameEncoder);
            recordingServer.setSurfaceVideoEncoder(surfaceVideoEncoder);

            recordingServer.setSurfaceEncoder(surfaceFrameEncoder);
            asyncProcessors.add(surfaceFrameEncoder);
            //asyncProcessors.add(surfaceVideoEncoder);
    ...
    private static void internalMain(String... args) throws Exception {
        ...
        try {
            if(options.isBoot()){
                ...
                p.preRun();
                p.runU2();
                p.runScrcpy(options.getPort());
                Ln.i("boot!");
            }else if(options.isWs()){
                scrcpy_ws(options);
            }else if(options.isKill()){
                Ln.i("kill...");
                ProcessManager p = new ProcessManager();
                p.preRun();
                Ln.i("kill end");
