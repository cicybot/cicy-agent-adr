...
public final class StreamerVideo {
    ...
    private final RecordingServer recordingServer;

    private final ByteBuffer headerBuffer = ByteBuffer.allocate(12);

    public StreamerVideo( RecordingServer recordingServer,Codec codec, boolean sendCodecMeta, boolean sendFrameMeta) {
        this.codec = codec;
    ...
    public ByteBuffer getVideoHeader(Size videoSize) throws IOException {
    ...
    public void writeVideoHeader(Size videoSize) throws IOException {
        if (sendCodecMeta) {
            writeFully(getVideoHeader(videoSize));
        }
    }
    ...
    public void writeDisableStream(boolean error) throws IOException {
        ...
        writeFullyBytes(code, 0, code.length);
    }
    ...
    private void writeFully(ByteBuffer buffer) {
        ...
        buffer.get(data);
        this.recordingServer.writeVideoScreen(data);
    }

    private void writeFullyBytes(byte[] code, int i, int length) {
        this.recordingServer.writeVideoScreen(code);
    }
